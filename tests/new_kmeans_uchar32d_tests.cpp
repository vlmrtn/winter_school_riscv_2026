...
namespace {
    ...
    class SyntheticTestKMeansStudent2_u8c32_new : public testing::TestWithParam<KMeansUniversalParams> {
    protected:
    ...
    void SyntheticTestKMeansStudent2_u8c32_new::run(const KMeansUniversalParams& params) {
        ...
        setRNGSeed(2026);
        double compactnessStud = kmeans_uchar32d(pointsU8Shuffled, clustersCount, labels,
        ...
        if (perf) {
            ...
            for (int i = 0; i < PERF_ITERATIONS; ++i) {
                kmeans_uchar32d(pointsU8Shuffled, clustersCount, labels,
                    criteria, attempts, flags, centers);
            }

            auto end = std::chrono::high_resolution_clock::now();
            const auto duration_st = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
            std::cout << "Student2 new_kmeans (32D) took " << duration_st.count() / PERF_ITERATIONS << " ms " << std::endl;
    ...
    TEST_P(SyntheticTestKMeansStudent2_u8c32_new, Basic) {
        const KMeansUniversalParams params = GetParam();
        run(params);
    }


    static std::vector<KMeansUniversalParams> synthetic_data_kmeans_student2_u8c32_new = {
    ...
    static std::vector<KMeansUniversalParams> synthetic_data_kmeans_student2_u8c32_new_perf = {
